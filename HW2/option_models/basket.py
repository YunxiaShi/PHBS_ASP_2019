...
def basket_price_mc(
    ...
    if( bsm ) :
        prices = forward[:,None] * np.exp((-1/2 * texp * cov_m.diagonal())[:, None] + np.sqrt(texp) * chol_m @ znorm_m)
    else:
...
def basket_price_norm_analytic(
    ...
    divr_basket = weights @ divr
    price = normal_formula(strike, spot_basket, vol_basket, texp, intr, divr_basket, cp_sign)
